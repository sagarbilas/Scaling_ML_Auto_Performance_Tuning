...
def persistent_ytopt(H, persis_info, gen_specs, libE_info):
    ...
    while tag not in [STOP_TAG, PERSIS_STOP]:
        ...
        #ADDEED TO CALCULATE AND RECORD ELAPSED TIME TO RESULTS.CSV FILE
        #startMinusEnd = calc_in['sim_ended_time']-calc_in['sim_started_time']
        #print("\n\n startMinusEnd from ytopt_Asktell.py: ", startMinusEnd, '\n')
        #calc_in['startMinusEnd'] = startMinusEnd
        #print("\n\n calc_in= ", calc_in, '\n')
        
        #elapsed_secs = calc_in['sim_ended_time'] - start_time
        #calc_in['elapsed_secs'] = elapsed_secs

        if calc_in is not None:
            if len(calc_in):
                b = np.vstack(map(list, calc_in))
                with open('../../results_node8_rank4_worker5_large_checking1.csv', 'a') as f:
                    if first_write:
